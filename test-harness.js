...
function executeCommand(command, options = {}) {
	...
	let fullCommand
	if (
		command.startsWith(CLI_BIN) ||
		command.startsWith('./dist/index.js') ||
...
function testBuildCancellation() {
	...
	runTest(
		...
		() => {
			...
			const _lines = result.stdout.split('\n')
			buildIds.forEach((id, _idx) => {
	...
	if (queuedBuildId) {
		runTest(
			`Cancel queued build ${queuedBuildId}`,
			() => {
	...
	if (completedBuildId) {
		runTest(
			`Cancel completed build ${completedBuildId} (should fail)`,
			() => {
				...
				const hasError =
					result.stderr.includes('cannot be cancelled') ||
	...
	runTest(
		...
		() => {
			...
			const hasBillingInfo =
				result.stdout.includes('Billing') ||
