...
import { whop, sessionPath } from '../lib/whop.ts'
...
export async function initCommand(path: string = '.'): Promise<void> {
	...
	if (projectInfo.hasPackageJson) {
		console.log(chalk.green(`  ✓ ${projectInfo.projectType === 'nextjs' ? 'Next.js' : projectInfo.projectType} project detected`))
	...
	if (projectInfo.projectType === 'nextjs') {
		...
		if (!hasStandalone) {
			...
			if (success) {
				...
				console.log(chalk.yellow('  Please add `output: "standalone"` to your next.config.js manually'))
			}
		}
	}
	...
	try {
		...
		if (shouldInstall.toLowerCase() !== 'n') {
			const installSpinner = createSpinner(`Installing dependencies with ${projectInfo.packageManager}...`)
