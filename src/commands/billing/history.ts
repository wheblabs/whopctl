...
export async function billingHistoryCommand(appId?: number, months?: number): Promise<void> {
	try {
		...
		data.summaries.forEach((summary: any, index: number) => {
			...
			console.log(
				`  Function Invocations: ${chalk.cyan(summary.functionInvocations?.toLocaleString() || '0')}`,
			)
			console.log(
				`  Bandwidth: ${chalk.cyan(parseFloat(summary.bandwidthGb || '0').toFixed(2))} GB`,
			)
			console.log(
				`  Build Minutes: ${chalk.cyan(parseFloat(summary.buildMinutes || '0').toFixed(2))}`,
			)
